...
<div class="card">
    <div class="card-header d-flex justify-content-between">
        <h3 class="card-title">Certificate of Indigency List</h3>
        ...
        <table class="table table-hover table-striped table-bordered" id="tbl-list">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-center p-0">Middlename</th>
                    <th class="text-center p-0">Occupation</th>
                    <th class="text-center p-0">Remarks</th>
                </tr>
            </thead>
            <tbody>
                <?php 
                $sql = "SELECT *,(lastname) as fullname,(firstname) as fname  FROM indigency_list order by `fullname` asc";
                $qry = $conn->query($sql);
                $i = 1;
                ...
                <tr>
                    ...
                    <td class="py-0 px-1"><?php echo date('M d,Y', strtotime($row["date_created"])); ?></td>
                    <td class="py-0 px-1"><?php echo $row['fullname'] ?></td>
                    <td class="py-0 px-1"><?php echo $row['fname'] ?></td>
                    <td class="py-0 px-1"><?php echo $row['middlename'] ?></td>
                    <td class="py-0 px-1"><?php echo $row['occupation'] ?></td>
                    <td class="text-center py-0 px-1">
                        <div class="btn-group" role="group">
                            ...
                            <li><a class="dropdown-item view_data" data-id="<?php echo $row['indigency_id'] ?>" href="javascript:void(0)">View</a></li>
                            <li><a class="dropdown-item edit_data" data-id = '<?php echo $row['indigency_id'] ?>' href="javascript:void(0)">Edit</a></li>
...
<noscript>
    <div class="d-flex w-100 align-items-center">
        ...
        ?><?php echo $city['city'] ?></p>
            ...
            <p class="fw-bold text-center">Certificate of Indigency</p>
        </div>
        <div class="col-2">
...
<script>
    ...
    $(function(){
        ...
        $('#create_new').click(function(){
            uni_modal('Add Indigency',"manage_indigency.php",'mid-large')
        })
        $('.edit_data').click(function(){
            uni_modal('Edit Indigency\'s Details',"manage_indigency.php?id="+$(this).attr('data-id'),'mid-large')
        })
        $('.view_data').click(function(){
            uni_modal('Indigency',"view_indigency.php?id="+$(this).attr('data-id'),'mid-large')
        })
        $('.delete_data').click(function(){
            _conf("Are you sure to delete <b>"+$(this).attr('data-name')+"</b> from Indigency List?",'delete_data',[$(this).attr('data-id')])
        })
    ...
    function delete_data($id){
        ...
        $.ajax({
            url:'./../Actions.php?a=delete_indigency',
            method:'POST',
            data:{id:$id},
            dataType:'JSON',
            error:err=>{
