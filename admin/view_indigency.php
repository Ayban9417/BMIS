...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT *,(lastname) as fullname,(firstname) as fname FROM indigency_list where indigency_id = '{$_GET['id']}'");
...
<div class="container-fluid">
    <div id="outprint_modal">
        <p>
            <span class="ms-5"></span>This is to certify that <b><u class="px-1"><?php  session_start(); echo $fullname.', '.$fname.', '.$age ?></u></b> years old, Filipino Citizen, resident of   <?php   echo $city['baranggay'] ?> <?php  echo $city['city'] ?>, <?php  echo $city['province'] ?>.
        </p>
        <p><span class="ms-5"></span>Further certify that the above-named person belongs to the Indigent Family in this Barangay.<br><br>
        This Certification is being issued upon the request of the interested party connection with the requirements for whatever legal purpose that may serve them best.	
</p>
        <p><span class="ms-5">ISSUED this <b><u class="px-1"><?php echo date('dS',strtotime($date_created)) ?></u></b> day of <b><u class="px-1"><?php echo date('F Y',strtotime($date_created)) ?></u></b> at <?php   echo $city['baranggay'] ?> <?php  echo $city['city'] ?>, <?php  echo $city['province'] ?> upon request of the interested party for whatever legal purposes it may serve.</p>
        <br>
        <br>
        <br>
        <br>
    ...
    <div class="row justify-content-end border-top pt-2">
        ...
        <div class="col-auto">
            <button class="btn btn-sm btn-dark rounded-0" data-bs-dismiss='modal' type="button"  onclick="history.go(-1);"><i class="fa fa-times"></i> Close</button>
        </div>
    </div>    
